...
def plot_bins(data,title,image_paths):
    ...
    for i,(x,image_path) in enumerate(zip(clipped_data,image_paths)):
        ...
        histograms.append(h)
        if len(data)>1:
            plt.bar([y+i*bar_width for y in range(len(bins)-1)],h, width=bar_width, edgecolor='k',label=image_path)
        else:
            plt.bar([y for y in range(len(bins)-1)],h, width=bar_width, edgecolor='k')
    # plt.hist(clipped_data, bins=bins, edgecolor="black",label=image_paths)

    if len(data)>1:
        plt.legend(loc='upper right')

    plt.xticks(ticks=[x -bar_width/2 for x in range(len(bins))],labels=bins[:-1]+['>2'])
    ...
    if len(data)>1:
        for i,(b1,b2) in enumerate(zip(bins[:-1],bins[1:-1]+['>2'])):
            ...
            if b1==bins[-2]:
                ...
                fig_title=f'Comparing configurations for droplets of size {b1} - {b2} µm'
            ...
            plt.bar(range(len(image_paths)),h, width=0.5, edgecolor='k',label=image_path)
